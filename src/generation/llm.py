...
def synthesize(context: str, query: str, language: str = "English", model_override: str | None = None) -> str:
    ...
    system = (
        ...
        "- WORKED EXAMPLES: If the context contains worked examples (e.g. 'Example "
        "1.5', 'Example 1.6'), reproduce EACH one faithfully and completely — its "
        "setup, the solution steps, the formulas used, and the FINAL RESULT (e.g. "
        "'net force = 0 by symmetry'). Keep distinct examples separate (do not merge "
        "Example 1.5 with 1.6) and do not omit, shorten, or paraphrase away an example "
        "that is present. Preserve their exact numbering and conclusions.\n"
        "- FIDELITY: Stay faithful to the source — include the key definitions, "
        "equations, examples and results that appear in the context. Do not add "
        "outside facts, and do not drop important source content to be brief.\n"
